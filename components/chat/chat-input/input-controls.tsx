...
export interface InputControlsProps {
  ...
  // Attachment-aware model filtering
  ...
  // Thread-level attachments for model filtering
...
export function InputControls({
  ...
  let availableModels = allAvailableModels.filter((modelId) =>
    enabledModels.has(modelId),
  );

  // Combine current input attachments with thread-level attachments for filtering
  const allAttachments = [...attachmentPreviews, ...threadAttachments];

  // If there are any attachments (current input OR in thread history), only show models that support file attachments
  if (allAttachments.length > 0) {
    const compatibleModels = availableModels.filter((modelId) => {
      const modelInfo = getModelInfo(modelId);
      return modelInfo.capabilities.fileAttachments;
    });

    // If we have compatible models, use them
    if (compatibleModels.length > 0) {
      availableModels = compatibleModels;

      // Auto-switch to a compatible model if current model doesn't support file attachments
      const currentModelInfo = getModelInfo(selectedModel);
      const isCurrentModelCompatible =
        currentModelInfo.capabilities.fileAttachments;

      if (!isCurrentModelCompatible) {
        // Switch to the first available compatible model
        setSelectedModel(compatibleModels[0]);
      }
    }
    // If no compatible models are available, keep all models but show warning
    ...
    <>
      ...
      <div className="flex items-center justify-between gap-2 mt-2 px-1">
        ...
        <div className="flex items-center gap-1 sm:gap-2 overflow-x-auto flex-1 min-w-0">
          ...
          {/* File Upload Buttons - Only show for models that support file attachments */}
          {currentModelInfo.capabilities.fileAttachments && (
            <>
              ...
              {/* PDF/Document Upload Button - Available for all file-supporting models */}
              <Button
                size="icon"
                variant="ghost"
